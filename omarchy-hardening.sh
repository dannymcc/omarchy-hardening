...
# DNSCrypt server options (popular public servers)
declare -A DNSCRYPT_SERVERS
DNSCRYPT_SERVERS[quad9]="quad9-dnscrypt-ip4-filter-pri"
DNSCRYPT_SERVERS[cloudflare]="cloudflare"
DNSCRYPT_SERVERS[mullvad]="mullvad-all"
DNSCRYPT_SERVERS[adguard]="adguard-dns"

# Configuration values (can be overridden via 'c' menu)
DNS_PROVIDER="quad9"
DNSCRYPT_SERVER="quad9"
...
show_menu() {
    ...
    local idx=1
    for key in llmnr firewall tailscale faillock git dns dnscrypt; do
        local label=""
        local desc=""
        case $key in
            llmnr)
            ...
            dns)
                ...
                ;;
            dnscrypt)
                label="Enable DNSCrypt"
                desc="Encrypts DNS via DNSCrypt protocol using ${DNSCRYPT_SERVER}"
                ;;
        esac
...
toggle_option() {
    local idx=$1
    local keys=(llmnr firewall tailscale faillock git dns dnscrypt)
...
configure_options() {
    ...
    # DNSCrypt Server
    echo -e "  ${CYAN}DNSCrypt Server${NC} ${DIM}(for DNSCrypt)${NC}"
    echo -e "  Current: ${BOLD}$DNSCRYPT_SERVER${NC}"
    echo -e "  ${DIM}Options: quad9, cloudflare, mullvad, adguard${NC}"
    echo -e "  ${DIM}  quad9     - Privacy-focused, blocks malware (recommended)${NC}"
    echo -e "  ${DIM}  cloudflare - Fast, privacy policy${NC}"
    echo -e "  ${DIM}  mullvad   - Privacy-focused, no logging${NC}"
    echo -e "  ${DIM}  adguard   - Blocks ads and trackers${NC}"
    echo -e "  ${DIM}Full list: https://dnscrypt.info/public-servers/${NC}"
    read -p "  New server (Enter to keep): " input
    [[ -n "$input" ]] && DNSCRYPT_SERVER="$input"
    echo ""
...
harden_dnscrypt() {
    echo ""
    echo -e "  ${BOLD}Configuring DNSCrypt...${NC}"
    ...
    if ! command -v dnscrypt-proxy &> /dev/null; then
        print_info "Installing dnscrypt-proxy..."
        sudo pacman -S --noconfirm dnscrypt-proxy
    fi

    # Check if Tailscale is available for split DNS
    ...
    print_info "Configuring dnscrypt-proxy..."

    # Build the config with optional forwarding rules
    local forwarding_line=""
    if [[ "$tailscale_available" == true ]]; then
        forwarding_line="forwarding_rules = '/etc/dnscrypt-proxy/forwarding-rules.txt'"
    fi

    sudo tee /etc/dnscrypt-proxy/dnscrypt-proxy.toml > /dev/null << EOF
# DNSCrypt-proxy configuration
# Generated by omarchy-hardening

listen_addresses = ['127.0.0.1:53', '[::1]:53']
server_names = ['$server_name']
$forwarding_line

# Use servers with DNSSEC, no logging, no filtering (unless you chose adguard)
require_dnssec = true
require_nolog = true
require_nofilter = false

# Fallback resolvers (used to retrieve server lists)
...
EOF
    ...
    # Create forwarding rules for Tailscale split DNS
    if [[ "$tailscale_available" == true ]]; then
        print_info "Configuring split DNS for Tailscale..."
        sudo tee /etc/dnscrypt-proxy/forwarding-rules.txt > /dev/null << 'EOF'
# Forward Tailscale domains to Tailscale's DNS
ts.net 100.100.100.100
EOF
        print_success "Split DNS configured - .ts.net queries go to Tailscale"
    fi
    ...
    sudo tee /etc/systemd/resolved.conf.d/dnscrypt.conf > /dev/null << 'EOF'
[Resolve]
DNS=127.0.0.1
DNSStubListener=no
EOF
    ...
    print_info "Updating resolv.conf..."
    ...
    print_success "DNSCrypt enabled using $server_name"
}
...
confirm_selection() {
    ...
    if [[ "${OPTIONS[dnscrypt]}" == true ]]; then
        local server_name="${DNSCRYPT_SERVERS[$DNSCRYPT_SERVER]:-$DNSCRYPT_SERVER}"
        echo -e "  ${GREEN}✓${NC} ${BOLD}Enable DNSCrypt${NC}"
        echo -e "    ${CYAN}Why:${NC} DNSCrypt authenticates and encrypts DNS traffic, preventing"
        echo -e "         tampering and eavesdropping on your DNS queries."
        echo -e "    ${CYAN}Server:${NC} ${BOLD}$server_name${NC}"
        echo -e "    ${CYAN}Action:${NC}"
        echo -e "      ${YELLOW}1.${NC} Install dnscrypt-proxy (if not present)"
        ...
        if command -v tailscale &> /dev/null && tailscale status &> /dev/null; then
            echo -e "      ${YELLOW}5.${NC} Disable Tailscale MagicDNS (--accept-dns=false)"
            echo -e "      ${YELLOW}6.${NC} Configure split DNS (.ts.net → Tailscale DNS)"
        fi
